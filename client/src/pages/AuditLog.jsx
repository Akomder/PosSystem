  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">Audit Trail</h2>
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">Activity log for all user actions</p>
        </div>
        <div className="flex items-center gap-2">
          <DateRangeFilter value={dateRange} onChange={setDateRange} />
          <Button size="sm" icon={RefreshCw} variant="secondary" onClick={load}>Refresh</Button>
        </div>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        >All</button>
        {ENTITY_TYPES.map(t => (
          <button key={t} onClick={() => setEntity(t)}
      ...
      {loading ? (
        ...
        <EmptyState icon={Shield} title="No audit logs" description="Activity will be recorded here" />
      ) : (
