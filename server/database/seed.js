/**
 * database/seed.js — Bella Vista POS
 *
 * Usage:
 *   node database/seed.js            Seed test data only (schema must already exist)
 *   node database/seed.js --setup    Apply posdb.sql schema first, then seed
 *
 * npm shortcuts (from server/):
 *   npm run seed                     Seed only
 *   npm run setup                    Schema + seed  (use on a fresh database)
 *
 * What it does:
 *   1. [--setup]  Executes posdb.sql against the connected database.
 *   2. [always]   Ensures the SuperAdmin account exists (safe to re-run).
 *   3. [always]   Truncates core tables and inserts test data:
 *                   3 users · 26 menu items · 20 tables · 10 orders · 6 staff
 */
...
const POSDB_SQL   = path.join(__dirname, 'posdb.sql')
const RESTAURANT  = 1   // default restaurant id (seeded by posdb.sql)
...
const ORDER_SEEDS = [
  { tableNum:  3, waiter: 'Sophie Martin', status: 'In Progress', notes: 'Allergy: nuts',         items: [{ name: 'Caesar Salad',    qty: 2, price: 12.50 }, { name: 'Grilled Salmon',   qty: 1, price: 24.00 }, { name: 'Sparkling Water', qty: 3, price: 3.50 }] },
  { tableNum:  7, waiter: 'James Wilson',  status: 'Pending',     notes: '',                       items: [{ name: 'Bruschetta',       qty: 1, price:  9.00 }, { name: 'Beef Tenderloin',  qty: 2, price: 34.00 }, { name: 'Red Wine',        qty: 2, price: 12.00 }] },
  { tableNum: 12, waiter: 'Emily Chen',    status: 'Served',      notes: 'Extra cheese on pizza',  items: [{ name: 'Tomato Soup',      qty: 2, price:  8.50 }, { name: 'Margherita Pizza', qty: 1, price: 18.00 }, { name: 'Tiramisu',        qty: 2, price: 8.00 }] },
  { tableNum:  5, waiter: 'Sophie Martin', status: 'In Progress', notes: '',                       items: [{ name: 'Caprese Salad',    qty: 1, price: 11.00 }, { name: 'Pasta Carbonara',  qty: 2, price: 16.50 }, { name: 'Lemonade',        qty: 2, price: 4.50 }] },
  { tableNum: 11, waiter: 'Sophie Martin', status: 'Pending',     notes: '',                       items: [{ name: 'Bruschetta',       qty: 2, price:  9.00 }, { name: 'Fish & Chips',     qty: 1, price: 19.00 }, { name: 'Sparkling Water', qty: 2, price: 3.50 }] },
  { tableNum: 14, waiter: 'James Wilson',  status: 'Pending',     notes: 'One chicken well done',  items: [{ name: 'Onion Soup',       qty: 3, price:  9.00 }, { name: 'Chicken Breast',   qty: 3, price: 22.00 }] },
  { tableNum:  2, waiter: 'Emily Chen',    status: 'In Progress', notes: '',                       items: [{ name: 'Caesar Salad',    qty: 1, price: 12.50 }, { name: 'Seafood Risotto',  qty: 2, price: 26.00 }, { name: 'Espresso',        qty: 2, price: 3.50 }] },
  { tableNum: 16, waiter: 'Marcus Lee',    status: 'Served',      notes: 'Vegan patty',            items: [{ name: 'Garlic Bread',     qty: 2, price:  6.00 }, { name: 'Veggie Burger',    qty: 2, price: 14.00 }, { name: 'Chocolate Mousse', qty: 2, price: 7.50 }] },
  { tableNum:  6, waiter: 'James Wilson',  status: 'In Progress', notes: 'Birthday dinner',        items: [{ name: 'Caprese Salad',    qty: 2, price: 11.00 }, { name: 'Grilled Salmon',   qty: 2, price: 24.00 }, { name: 'Creme Brulee',    qty: 2, price: 8.50 }] },
  { tableNum: 18, waiter: 'Emily Chen',    status: 'Served',      notes: '',                       items: [{ name: 'Beef Tenderloin',  qty: 1, price: 34.00 }, { name: 'Red Wine',         qty: 2, price: 12.00 }] },
]
...
async function runSchema() {
  ...
  console.log('  ✓ posdb.sql applied')
}

// ── Phase 2: SuperAdmin upsert ────────────────────────────────────────────────
// Runs outside the data transaction so the account survives re-seeds.
...
async function ensureSuperAdmin() {
  ...
  console.log('  ✓ Created  superadmin@pos.com / superadmin123')
}
...
async function seedData() {
  ...
  try {
    ...
    await client.query(
      `TRUNCATE order_items, orders, restaurant_tables, menu_items, staff, users
    ...
    const { rows: userRows } = await client.query(`
      INSERT INTO users (restaurant_id, email, password_hash, role) VALUES
        ($1, 'admin@pos.com',   $2, 'Admin'),
        ($1, 'waiter@pos.com',  $3, 'Waiter'),
        ($1, 'cashier@pos.com', $4, 'Cashier')
      RETURNING id
    `, [RESTAURANT, adminHash, waiterHash, cashierHash])
    const [adminId, waiterId, cashierId] = userRows.map(r => r.id)
    console.log('  ✓ Users          (3)')
    ...
    for (const item of MENU_ITEMS) {
      const { rows } = await client.query(`
        INSERT INTO menu_items
          (restaurant_id, name, category, price, description, available, stock, tags, prep_time)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
        RETURNING id
      `, [RESTAURANT, item.name, item.category, item.price, item.description,
          item.available, item.stock, item.tags, item.prep_time])
      menuIds[item.name] = rows[0].id
    }
    console.log(`  ✓ Menu items     (${MENU_ITEMS.length})`)

    // Tables
    const tableIds = {}   // table number → db id
    for (const t of TABLES) {
      const { rows } = await client.query(`
        INSERT INTO restaurant_tables
          (restaurant_id, number, capacity, status, waiter, section, position_row, position_col)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
        RETURNING id
      `, [RESTAURANT, t.number, t.capacity, t.status, t.waiter,
          t.section, t.position_row, t.position_col])
      tableIds[t.number] = rows[0].id
    }
    ...
    for (const o of ORDER_SEEDS) {
      ...
      const { rows: ordRows } = await client.query(`
        INSERT INTO orders
          (restaurant_id, table_id, table_number, waiter, status, subtotal, tax, total, notes)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
        RETURNING id
      `, [RESTAURANT, tableDbId, o.tableNum, o.waiter, o.status,
          subtotal, tax, total, o.notes])
      const ordId = ordRows[0].id

      for (const item of o.items) {
        const miId = menuIds[item.name]
        if (!miId) throw new Error(`Unknown menu item in ORDER_SEEDS: "${item.name}"`)
        await client.query(`
          INSERT INTO order_items
            (restaurant_id, order_id, menu_item_id, name, quantity, unit_price)
          VALUES ($1,$2,$3,$4,$5,$6)
        `, [RESTAURANT, ordId, miId, item.name, item.qty, item.price])
      }

      // Link occupied/served tables to their current order
    ...
    console.log(`  ✓ Orders         (${ORDER_SEEDS.length}) with line items`)

    // Staff
    await client.query(`
      INSERT INTO staff (restaurant_id, user_id, name, role, tables_assigned) VALUES
        ($1, $2, 'Sophie Martin', 'Waiter',  '{3,5,11}'),
        ($1, NULL,'James Wilson',  'Waiter',  '{6,7,14}'),
        ($1, $3, 'Emily Chen',    'Waiter',  '{2,12,18}'),
        ($1, NULL,'Marcus Lee',    'Waiter',  '{16}'),
        ($1, $4, 'Sarah Kim',     'Cashier', '{}'),
        ($1, $5, 'David Brown',   'Admin',   '{}')
    `, [RESTAURANT, waiterId, waiterId, cashierId, adminId])
    console.log('  ✓ Staff          (6)')
...
async function main() {
  console.log('Bella Vista POS — Database Seed')
  console.log(`Mode: ${SETUP_MODE ? '--setup (schema + seed)' : 'seed only'}`)

  try {
    if (SETUP_MODE) await runSchema()
    await seedData()
