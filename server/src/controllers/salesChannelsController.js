...
async function getAll(req, res, next) {
  try {
    const { rows } = await query(
      'SELECT * FROM sales_channels WHERE restaurant_id = $1 ORDER BY name',
      [req.restaurantId]
    )
...
async function getOne(req, res, next) {
  try {
    const { rows } = await query(
      'SELECT * FROM sales_channels WHERE id = $1 AND restaurant_id = $2',
...
async function create(req, res, next) {
  ...
  try {
    const { rows } = await query(
      `INSERT INTO sales_channels (restaurant_id, name, description)
       VALUES ($1,$2,$3) RETURNING *`,
      [req.restaurantId, name, description || '']
    )
...
async function update(req, res, next) {
  ...
  try {
    const { rows } = await query(
      `UPDATE sales_channels SET ${sets.join(', ')}, updated_at = NOW()
...
async function remove(req, res, next) {
  try {
    const { rowCount } = await query(
      'DELETE FROM sales_channels WHERE id = $1 AND restaurant_id = $2',
      [req.params.id, req.restaurantId]
    )
