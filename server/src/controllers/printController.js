...
function formatDateOnly(iso, locale = "en-US", dateStyle = "medium") {
...
function buildReceiptHtml(order, restaurant, isKitchen = false) {
  ...
  const nameFs =
    cfg.restaurantNameSize === "xlarge"
      ? fontSize + 6
      : cfg.restaurantNameSize === "normal"
        ? fontSize + 2
        : fontSize + 4; // 'large'

  // Divider CSS
  const divCss =
    cfg.dividerStyle === "solid"
  ...
  // ── Payment rows ───────────────────────────────────────────────────────────
  const paymentRows =
    !isKitchen && cfg.showPaymentDetails && (order.payments || []).length
      ? (order.payments || [])
          .map(
            (p) => `
  ...
  // ── Signature line ─────────────────────────────────────────────────────────
  const signatureLine =
    !isKitchen && cfg.showSignatureLine
      ? `${cfg.showDividers ? dividerEl : ""}
  ...
  const altCurrencyBlock =
    !isKitchen && cfg.showAltCurrencies && (usdRate || thbRate)
      ? `${cfg.showDividers ? dividerEl : ""}
       <div style="font-size:${fontSize - 1}px;">
  ...
  // ── QR payment ─────────────────────────────────────────────────────────────
  const qrImageBase64 =
    (restaurant.settings || {}).payment?.qrImageBase64 || "";
  const qrPaymentBlock =
    !isKitchen && cfg.showQrPayment
      ? `${cfg.showDividers ? dividerEl : ""}
  ...
  const metaLines = [
    cfg.showOrderId
      ? `${lbl(cfg, "labelOrder", "Order")}: ${escHtml(order.id)}`
      : "",
    cfg.showTableNumber && order.tableNumber
      ? `${lbl(cfg, "labelTable", "Table")}: ${order.tableNumber}`
      : "",
    cfg.showOrderType && order.orderType
      ? `${lbl(cfg, "labelOrderType", "Type")}: ${escHtml(order.orderType)}`
      : "",
...
function buildReturnHtml(ret, restaurant) {
  const cur = restaurant.currency || "LAK";
  const date = formatDateOnly(ret.createdAt);
...
function buildPurchaseHtml(po, restaurant) {
  const cur = restaurant.currency || "LAK";
  const date = formatDateOnly(po.orderedAt || po.createdAt);
...
<div style="display:flex;gap:20px;flex-wrap:wrap;margin:6px 0;">
  ...
  <div style="flex:1">
    ...
    ${po.expectedAt ? `<div class="info-row"><span class="lbl">Expected:</span> ${formatDateOnly(po.expectedAt)}</div>` : ""}
    ${po.receivedAt ? `<div class="info-row"><span class="lbl">Received:</span> ${formatDateOnly(po.receivedAt)}</div>` : ""}
  </div>
</div>
<div class="divider"></div>
<table>
...
function buildPurchaseReturnHtml(pr, restaurant) {
  const cur = restaurant.currency || "LAK";
  const date = formatDateOnly(pr.createdAt);
...
function buildCashInHtml(entry, restaurant) {
  const cur = restaurant.currency || "LAK";
  const date = formatDateOnly(entry.createdAt, "en-US", "long");
  const body = `
...
async function getDraftOrder(req, res, next) {
  try {
    ...
    res
      ...
      .send(buildDraftHtml(order, restaurant));
  } catch (err) {
    next(err);
  }
}
...
async function getReturnInvoice(req, res, next) {
  try {
    ...
    const r = rows[0];
    const ret = {
      id: `RET-${String(r.id).padStart(4, "0")}`,
      _id: r.id,
      orderId: r.order_id,
      reason: r.reason,
    ...
    res
      .set("Content-Type", "text/html; charset=utf-8")
      .send(buildReturnHtml(ret, restaurant));
  } catch (err) {
    next(err);
  }
}
...
async function getPurchaseReceipt(req, res, next) {
  try {
    ...
    res
      .set("Content-Type", "text/html; charset=utf-8")
      .send(buildPurchaseHtml(data, restaurant));
  } catch (err) {
    next(err);
  }
}
...
async function getPurchaseReturnReceipt(req, res, next) {
  try {
    ...
    const data = {
      id: `PR-${String(pr.id).padStart(4, "0")}`,
      _id: pr.id,
    ...
    res
      .set("Content-Type", "text/html; charset=utf-8")
      .send(buildPurchaseReturnHtml(data, restaurant));
  } catch (err) {
    next(err);
  }
}
...
async function getCashFlowPrint(req, res, next) {
  try {
    const rid = req.restaurantId;
    const { rows } = await query(
      "SELECT * FROM cash_flow_entries WHERE id=$1 AND restaurant_id=$2",
      [req.params.id, rid],
    );
    if (!rows.length)
